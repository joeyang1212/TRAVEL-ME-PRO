...
export function ShoppingWinePreview() {
  ...
  return (
    <main className="appShell">
      <section className="hero"><div><small>ULTIMATE CANDIDATE</small><h1>Travel ME Ultimate</h1><p>Today、Weather、Shopping、Wine 與 Gemini 已開始整合到同一個候選頁。</p></div><div className="summaryCards"><article><small>Day</small><strong>{day}</strong></article><article><small>城市</small><strong>{trip.city}</strong></article><article><small>購物車</small><strong>{cartCount}</strong></article><article><small>酒款資料</small><strong>{nzWines.length}</strong></article></div></section>

      <TodayDashboard day={day} trip={trip} weatherLabel={currentWeather ? `${currentWeather.temperature}°C` : weather.loading ? "更新中" : "尚未取得"} humidity={currentWeather?.humidity} photoTotal={0} photoDone={0} recommendationCount={recommendationCount} cartTotalTwd={cartTotalTwd} remainingBudget={budget} projectedBudget={remainingBudget} aiUsage={0} aiLimit={30} onDayChange={setDay} itinerary={nzItinerary} />
